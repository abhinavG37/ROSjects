...
Each sensor stream is stored in a single folder. The main folder (Example: 2011_09_26_drive_0001_sync/image_00) contains
meta information(data/) and a timestamp file(timestamps.txt), listing the timestamp of each frame
...
number of points (WHY? THERE CAN BE n<N points WHICH POTENTIALLY DO NOT GET REFLECTED
				OFF AN OBSTACLE AND GET REGISTERED ON THE SENSOR),
			 this must be determined from the file size when reading
...
which are triggered at a certain point in time).
...
The time-stamps for the beginning and the end of the sweeps can be found in the time-
